...
fn setter_getter_test()
{
    ...
    let mut solver: BogackiShampine32<f64> =
...
fn fn1()
{
    ...
    assert_relative_eq!(*init_cond.get(0) * (2.0 * time_span.1).exp(), *y[len - 1].get(0), epsilon=0.4);
}
...
fn fn2()
{
    ...
    assert_relative_eq!(time_span.1.tan(), *y[len - 1].get(0), epsilon=0.05);
}
