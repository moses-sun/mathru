use mathru::{
    ...
    analysis::differential_equation::ordinary::{ExplicitODE, FixedStepper, Ralston3},
};
...
fn fn1()
{
    ...
    let (t, y): (Vec<f64>, Vec<Vector<f64>>) = solver.solve(&problem, &Ralston3::default()).unwrap();
    ...
    assert_relative_eq!(init_cond[0] * (2.0 * time_span.1).exp(), y[len - 1][0], epsilon=0.00001);
}
...
fn fn2()
{
    ...
    assert_relative_eq!(time_span.1.tan(), y[len - 1][0], epsilon=0.0001);
}
