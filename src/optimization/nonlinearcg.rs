...
//}
//
///// The nonlinear conjugate gradient is a generalized method of the conjugate gradient method for nonlinear optimization problems.
///// In this module, the method of Polak and Ribiere is implemented.
/////
///// input: $` f \colon \mathbb{R}^{n} \to \mathbb{R} `$ with initial approximation $` x_{0} \in \mathbb{R}^{n} `$
/////
///// output: $` x_{k} `$
/////
///// 1. Initialization: $` \sigma \in (0, 1), 0 < \underline{\gamma} < 1 < \overline{\gamma} `$, set $` d_{0} := - \nabla f
///// (x_{0}), k := 0 `$
///// 2. $` \alpha_{k} := \frac{\lvert \nabla f(x_k)^{T}d_{k} \rvert}{\lvert \lvert d_{k} \rvert \rvert_{2}^{2}} `$
///// 3. $` x_{k+1} := x_k + \alpha_{k} d_k `$ <br>
/////     $` \beta_{k} := \frac{\nabla f(x_{k+1})^T(\nabla f(x_{k+1}) - \nabla f(x_k))}{\lvert \lvert \nabla f(x_k) \rvert \rvert_{2}^{2}}
///// `$ <br>
/////     $` d_{k+1} := - \nabla f(x_{k+1}) + \beta_k d_k `$
///// 4. If one of the following condition is not hold, halve $` \alpha_k `$ and go to 3 <br>
/////     $`f(x_{k+1}) \leq f(x_k) - \sigma \alpha_k^2 \lvert \lvert d_k \rvert \rvert_2^2 `$ <br>
/////     $`-\overline{\gamma}\lvert \lvert \nabla f(x_{k+1}) \rvert \rvert_{2}^{2} \leq \nabla f(x_{k+1})^Td_{k+1} \leq
///// -\underline{\gamma}\lvert \lvert \nabla f(x_{k+1}) \rvert \rvert_{2}^{2} `$
///// 5. if $` \nabla f(x_{k+1}) \neq 0 `$ increase $` k:= k + 1 `$ and go to 2
///// # Example
/////
/////
...
//
//            if r_n_1.dotp(&r_n_1).pow(&T::from_f64(0.5).unwrap()) < self.epsilon
//            {
//                break;
//            }
//
