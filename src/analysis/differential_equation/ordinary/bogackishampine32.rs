...
impl<T> BogackiShampine32<T> where T: Real
{
    ...
    pub fn new(n_max: u32,
               ...
               -> BogackiShampine32<T>
    {
        return BogackiShampine32 { stepper: AdaptiveStepper::new(n_max, h_0, fac, fac_min,
                                                                 fac_max, abs_tol, rel_tol) };
    }
...
impl<T> ExplicitAdaptiveMethod<T> for BogackiShampine32<T> where T: Real
{
        ...
        where F: ExplicitODE<T>
    {
        ...
        let k_2: Vector<T> = prob.func(&(*t_n + *h_n / T::from_f64(2.0)),
                                       &(x_n + &(&k_1 * &(*h_n / T::from_f64(2.0)))));

        //k_3 = f(t_n + 3/4h_n, x_n + 3/4h_nk_2)
        let k_3: Vector<T> = prob.func(&(*t_n + *h_n / T::from_f64(3.0 / 4.0)),
                                       &(x_n + &(&k_2 * &(*h_n / T::from_f64(3.0 / 4.0)))));

        // x_{n + 1} = x_n + 2/9h_nk_1 + 1/3h_nk_2 + 4/9h_nk_3
        let x_n1: Vector<T> = x_n
                              + &(&k_1 * &(*h_n * T::from_f64(2.0 / 9.0)))
                              + (&k_2 * &(*h_n * T::from_f64(1.0 / 3.0)))
        ...
        let y_n1: Vector<T> = (x_n + &(&k_1 * &(*h_n * T::from_f64(7.0 / 24.0))))
                              + (&k_2 * &(*h_n * T::from_f64(1.0 / 4.0)))
                              + (&k_3 * &(*h_n * T::from_f64(1.0 / 3.0)))
                              + (&k_4 * &(*h_n * T::from_f64(1.0 / 8.0)));
