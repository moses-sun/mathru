    ...
    T: Field + Scalar + Power + AbsDiffEq<Epsilon = T>,
{
    pub fn calc_eigenvector(self: &Self, eigen_values: &Vec<T>) -> General<T> {
        ...
        for (lambda_idx, lambda) in eigen_values.iter().enumerate() {
            for row in 0..eigen_values.len() {
                let sum = (0..row).fold(T::zero(), |s, i| {
