...
use crate::algebra::abstr::magma::{Magma, MagmaAdd, MagmaMul};
...
impl<T> Polynomial<T> {
        ...
        T: Field + Scalar + AbsDiffEq<Epsilon = T>,
    {
        ...
        match n {
            ...
            1 => Polynomial {
                coef: vec![T::zero(), T::one()],
            },
            _ => {
                let p_1 = Polynomial {
                    coef: vec![T::zero(), T::from_f64(2.0f64 * n_f64 - 1.0f64)],
                };
                ((p_1 * Polynomial::from_legendre(n - 1)
                    - Polynomial {
                        coef: vec![T::from_f64(n_f64 - 1.0f64)],
                    } * Polynomial::from_legendre(n - 2))
                    / (Polynomial {
                        coef: vec![T::from_f64(n_f64)],
                    }))
                .0
            }
        }
        ...
        T: Field + Scalar + AbsDiffEq<Epsilon = T>,
    {
        match n {
            ...
            _ => {
                let p_1 = Polynomial {
                    coef: vec![T::zero(), T::from_f64(2.0f64)],
                };
        ...
        T: Field + Scalar + AbsDiffEq<Epsilon = T>,
    {
        match n {
            ...
            1 => Polynomial {
                coef: vec![T::zero(), T::from_f64(2.0)],
            },
            _ => {
                let p_1 = Polynomial {
                    coef: vec![T::zero(), T::from_f64(2.0f64)],
    ...
    T: Field + Scalar + AbsDiffEq<Epsilon = T>,
{
    ...
    fn div(self, rhs: &'b Polynomial<T>) -> Self::Output {
        ...
        (
            ...
            Polynomial {
                coef: Polynomial::reduce_coef(remainder),
            },
        )
    }
}
    ...
    T: Zero + AbsDiffEq,
{
    ...
    pub fn reduce(self) -> Self {
        Polynomial {
            coef: Polynomial::reduce_coef(self.coef),
        }
    }
}
    ...
    T: Field + Scalar + AbsDiffEq<Epsilon = T>,
{
    ...
    pub fn differentiate(&self) -> Polynomial<T> {
        ...
        Polynomial {
            coef: Polynomial::reduce_coef(coef_diff),
        }
    }
...
where
    T: MagmaAdd + Magma<Addition> + Scalar,
{
...
impl<T> MagmaAdd for Polynomial<T> where T: MagmaAdd + Scalar {}

impl<T> Magma<Multiplication> for Polynomial<T>
where
    T: MagmaMul + MonoidAdd + Magma<Multiplication> + Scalar,
{
    fn operate(self, rhs: Self) -> Self {
        self.mul(rhs)
    }
...
impl<T> MagmaMul for Polynomial<T> where T: MagmaMul + MonoidAdd + Scalar {}

impl<T> Semigroup<Addition> for Polynomial<T> where T: MagmaAdd + Scalar {}

impl<T> SemigroupAdd for Polynomial<T> where
    T: SemigroupAdd + Semigroup<Addition> + MagmaAdd + Scalar
{
}
...
impl<T> Monoid<Addition> for Polynomial<T> where T: MagmaAdd + Scalar + Identity<Addition> {}

impl<T> MonoidAdd for Polynomial<T> where T: Monoid<Addition> + SemigroupAdd + Zero + Scalar {}

impl<T> Quasigroup<Addition> for Polynomial<T> where T: Quasigroup<Addition> + Scalar + MagmaAdd {}

impl<T> Loop<Addition> for Polynomial<T> where
    T: Loop<Addition> + Quasigroup<Addition> + Scalar + MagmaAdd
{
}

impl<T> Group<Addition> for Polynomial<T> where
    T: Group<Addition> + Loop<Addition> + Quasigroup<Addition> + Scalar + MagmaAdd
{
}
